  ...
  return (
    <PageContainer>
      ...
      <div className="p-4 space-y-6 pb-24">
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-[#E5F5E1]">
          ...
          <div className="bg-[#EFFFF0] p-3 rounded-xl flex items-start gap-2 border border-[#8EEA7A]">
            <Info size={16} className="text-[#58C85A] mt-0.5 shrink-0" />
          ...
          {MOCK_PUBLIC_DATA.map((data) => (
            <SoftCard key={data.id} className="space-y-4">
              <div className="flex justify-between items-start gap-2">
                <div>
                  <h2 className="font-bold text-lg text-[#222222] flex items-center gap-1.5 mb-1">
                    <Database size={18} className="text-[#58C85A]" />
                    {data.name}
                  </h2>
                  <p className="text-sm text-[#777777]">{data.provider}</p>
                </div>
              ...
              {data.url && (
                <a href={data.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs text-[#58C85A] hover:underline">
                  <LinkIcon size={12} />
                  출처 및 상세 데이터 보기
                </a>
              )}
              ...
              <div className="pt-2 grid grid-cols-2 gap-4">
                <div>
                  <span className="text-xs font-bold text-[#777777] block mb-1">활용 항목</span>
                  <div className="flex flex-wrap gap-1.5">
                    {data.usedFields.map((field, i) => (
                      <span key={i} className="text-xs bg-white border border-[#E5F5E1] text-[#222222] px-2 py-1 rounded">
                        {field}
                      </span>
                    ))}
                  </div>
                ...
                <div>
                  <span className="text-xs font-bold text-[#777777] block mb-1">서비스 내 활용</span>
                  <p className="text-sm text-[#222222]">{data.usage}</p>
                </div>
              </div>

              <div>
                <span className="text-xs font-bold text-[#777777] block mb-1">MVP 구현 방식</span>
