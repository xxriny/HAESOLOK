        ...
        <SoftCard className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-semibold mb-1 text-foreground">
              학교 검색
              <span className="ml-1 text-xs font-normal text-primary">나이스 연동</span>
            </label>
            ...
            {/* 선택된 학교 표시 */}
            {selectedSchool && (
              <div className="flex items-center gap-2 p-3 mb-2 rounded-xl bg-secondary border border-primary">
                <CheckCircle size={16} className="text-primary shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-foreground truncate">{selectedSchool.SCHUL_NM}</p>
              ...
              <>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 p-3 rounded-xl border border-border bg-white text-sm text-foreground outline-none focus:ring-1 focus:ring-primary"
                  />
                  <button
                    onClick={handleSearch}
                    disabled={isSearching}
                    className="px-4 py-3 rounded-xl bg-primary text-white text-sm font-medium hover:bg-primary/90 disabled:opacity-50 transition-all flex items-center gap-1"
                  >
                    {isSearching
                ...
                {searchResults.length > 0 && (
                  <div className="mt-2 border border-border rounded-xl overflow-hidden">
                    {searchResults.map((school, i) => (
                      ...
                      >
                        <p className="text-sm font-semibold text-foreground">{school.SCHUL_NM}</p>
